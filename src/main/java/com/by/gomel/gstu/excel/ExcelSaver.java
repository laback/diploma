...
public class ExcelSaver {
    ...
    private static String getFileName(){
        return LocalDateTime.now().toString().replace(":", ".").replace("T", " ");
    }

    public static void savePeriodOrderReport(List<Order> orders, User user) throws IOException {
        try(Workbook workbook = new HSSFWorkbook();
            var fos = new FileOutputStream(dir + "/reports/orderByDateReports/" + getFileName() + ".xls")){
    ...
    public static void saveDetailsSalesReport(List<SailedDetailsViewModel> sailedDetailsViewModels, User user) throws IOException {
        try(Workbook workbook = new HSSFWorkbook();
            var fos = new FileOutputStream(dir + "/reports/detailsSalesByPeriod/" + getFileName() + ".xls")){
            ...
            if(sailedDetailsViewModels == null || user == null || sailedDetailsViewModels.isEmpty()){
                Cell empty = row.createCell(0);
                empty.setCellValue("Нет данных");
            } else{
                ...
                reportFooter(user, sheet, sailedDetailsViewModels.size());
            }
    ...
    public static void saveEmployeePeriodReport(List<Order> orders, User user) throws IOException {
        try (Workbook workbook = new HSSFWorkbook();
             var fos = new FileOutputStream(dir + "/reports/employeePeriod/" + getFileName() + ".xls")) {
            Sheet sheet = workbook.createSheet("Заказы");
    ...
    private static void reportFooter(User user, Sheet sheet, int size) throws IOException {
    ...
    private static void formOrderReport(List<Order> orders, User user, Workbook workbook, FileOutputStream fos, Sheet sheet) throws IOException {
        ...
        if(orders == null || user == null || orders.isEmpty()){
            Cell empty = row.createCell(0);
            empty.setCellValue("Данный нет");
        } else{
            Cell customerFio = row.createCell(0);
            ...
            reportFooter(user, sheet, orders.size());
        }
